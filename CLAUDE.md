...
- M4 (privileged helper) is deferred by the user (5 September 2026): `SMAppService` daemons need a Team ID signature and this Mac only has a self-signed identity. Notarisation waits on the same decision; updates use the in-house ed25519 updater instead of Sparkle.
