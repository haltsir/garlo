...
/// Self-update from GitHub releases, ported from Vestitel. Once a day the
/// app asks the releases API for the newest version; a newer one is
/// downloaded together with its detached ed25519 signature (made by
/// Tools/sign-release.swift at release time), verified against the embedded
/// public key, unpacked, and staged. The swap waits until the popover is
/// closed: a helper script replaces the bundle after the app exits and
...
enum Updater {
    ...
    /// ~/.config/garlo/release-key on the release machine only.
    static let publicKeyBase64 = "W8+KerH3Vu4npLRPk6PvCGEtKB1DmR1LQGl7NpXbyd4="

    /// GARLO_UPDATE_URL points a test instance at a stand-in releases
    /// document (same JSON shape as the GitHub API).
    static let releasesAPI = URL(string:
        ProcessInfo.processInfo.environment["GARLO_UPDATE_URL"]
            ?? "https://api.github.com/repos/haltsir/garlo/releases/latest")!

    /// "1.2.3" against "1.2": true when b is strictly newer than a. Missing
    ...
    static func verify(zip: Data, signatureText: String) -> Bool {
        guard let keyRaw = Data(base64Encoded: publicKeyBase64),
              let signature = Data(base64Encoded: signatureText.trimmingCharacters(in: .whitespacesAndNewlines)),
              let key = try? Curve25519.Signing.PublicKey(rawRepresentation: keyRaw) else { return false }
...
extension AppStore {
    ...
    /// Updates are off for test instances (they would replace their own
    /// throwaway bundles) unless the stand-in API seam is set.
    var updaterAvailable: Bool {
        guard currentAppVersion != nil, Bundle.main.bundleURL.pathExtension == "app" else { return false }
        if ProcessInfo.processInfo.environment["GARLO_STATE_DIR"] != nil {
            return ProcessInfo.processInfo.environment["GARLO_UPDATE_URL"] != nil
        }
        return true
    }

    /// Rides the 30 s housekeeping tick: a one-shot timer aimed at 12:30
    ...
    func maybeRunDailyUpdateCheck() {
        ...
        // whatever the automatic-check setting says.
        if stagedUpdatePath != nil {
            installStagedUpdateIfIdle()
            return
        }
        guard settings.autoUpdateEnabled else { return }
        guard let trigger = Calendar.current.date(bySettingHour: Updater.dailyHour, minute: Updater.dailyMinute, second: 0, of: Date()),
    ...
    private func runUpdateCheck(manual: Bool) async {
        ...
        do {
            ...
            request.setValue("Garlo-Updater", forHTTPHeaderField: "User-Agent")
            ...
            guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                  ...
                  let assets = json["assets"] as? [[String: Any]] else { throw UpdateError.badResponse }
            ...
            guard Updater.verify(zip: zipData, signatureText: String(decoding: sigData, as: UTF8.self)) else {
                updateStatus = "Version \(remote) has an invalid signature; skipped."
                return
            }
            let staging = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("garlo-update-\(remote)", isDirectory: true)
            ...
            let newApp = staging.appendingPathComponent("Garlo.app")
    ...
    func installStagedUpdateIfIdle(force: Bool = false) {
        guard let staged = stagedUpdatePath, FileManager.default.fileExists(atPath: staged) else {
            stagedUpdatePath = nil
            return
        }
        if !force { guard !popoverOpen else { return } }
        let appPath = Bundle.main.bundleURL.path
        let pid = ProcessInfo.processInfo.processIdentifier
        // A test instance must come back as a test instance: relaunch the
        // binary with its state dir instead of `open`.
        let relaunch: String
        if let stateDir = ProcessInfo.processInfo.environment["GARLO_STATE_DIR"] {
            let updateURL = ProcessInfo.processInfo.environment["GARLO_UPDATE_URL"] ?? ""
            relaunch = "GARLO_STATE_DIR=\"\(stateDir)\" GARLO_UPDATE_URL=\"\(updateURL)\" nohup \"\(appPath)/Contents/MacOS/Garlo\" >/dev/null 2>&1 &"
        } else {
        ...
        \(relaunch)
        """
        let scriptFile = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("garlo-swap.sh")
        do {
    ...
    func noteVersionChange() {
        ...
        if let last = lastRunVersion, let current, last != current {
            Notifier.shared.postUpdated(version: current)
        }
        // the staged bundle is gone once it has been swapped in
        if let staged = stagedUpdateVersion, let current, Updater.sameVersion(staged, current) {
            stagedUpdatePath = nil
    ...
    private enum UpdateError: Error {
        case badResponse, processFailed, http(Int)
    }
