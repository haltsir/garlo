...
- History and Settings close the popover as they open (`ContentView.closePopover`, a click on the status item button): the popover floats at menu level, so a window opened under it stays hidden behind it. Do not close the `MenuBarExtraWindow` directly; SwiftUI then still believes it is shown and the next click on the icon does nothing.
- Now rows: fixed order disks, link, CPU, memory; the popover never shrinks while open. A row is name and label on one line and the figure on a full-width line of its own, never truncated. A click expands it to the three heaviest processes (`AppStore.nowDetail(for:)`, computed only while a row is open, so collapsed rows cost nothing per tick). With `showAllNow` (default on) every resource is listed and idle ones say "idle" (`NowItem.active` false); with it off rows take 2 s to appear and stay 10 s. Do not undo the hysteresis for a cosmetic change; it was the 0.2.2 fix.
